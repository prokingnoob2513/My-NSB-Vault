...
def str_to_eval(r):
    ...
    r = re.sub(r'(.+)\!', r'fact(\1)', r)

    r = re.sub(r"(.+)\{(.+)\}(.+)", r"arrow(\1, \2, \3)", r)
    r = re.sub(r"(.+)\^\^(.+)", r"tetr(\1, \2)", r)
    r = re.sub(r"(.+)\^(.+)", r"pow(\1, \2)", r)
    r = re.sub(r"(.+)\*(.+)", r"mul(\1, \2)", r)
    r = re.sub(r"(.+)\/(.+)", r"div(\1, \2)", r)
    r = re.sub(r"(.+)\+(.+)", r"add(\1, \2)", r)
    r = re.sub(r"(.+)\-(.+)", r"sub(\1, \2)", r)
    
    r = re.sub(r"(.+)\<(.+)", r"lt(\1, \2)", r)
    r = re.sub(r"(.+)\<\=(.+)", r"lte(\1, \2)", r)
    r = re.sub(r"(.+)\>(.+)", r"gt(\1, \2)", r)
    r = re.sub(r"(.+)\>\=(.+)", r"gte(\1, \2)", r)
    """
